  ...
  for(argcount=1; argcount<argc; argcount+=skip)
      ...
      if(argv[argcount][0] == '-')
	{
	  ...
	  switch(argv[argcount][1])
	    {
	    case 'N': /* Scanning Paramter Space No_of_Points * No_of_Points */
	    ...
	    case 'P': /* Population Size */
	      ...
	      skip++;
	      break;
	    case 'R': /* Control Parameter of Resilience  */
	      sscanf(argv[argcount+1],"%d", &RESILIENCE);
	      skip++;
	      break;
	    ...
	    case 'h':
	      ...
	      printf(" -R %d >> Control Parameter for Resilience\n",RESILIENCE);
	      printf("       >> R = 0,1\n");
              printf("       >> R=0 (Discret Values) R=1 (Continuous Values)\n");
	      printf("\n");
