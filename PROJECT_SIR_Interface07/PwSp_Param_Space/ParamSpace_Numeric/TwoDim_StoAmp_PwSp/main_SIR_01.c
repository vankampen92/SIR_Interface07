...
int main(int argc, char **argv)
{
  ...
  double d_Rel, b_Rel, coherence, f_peak;
  float relative_coherence, value;
  ...
  POPULATION = 100000; COHERENCE = 0; f_semiband = 1.E-1; 
  ...
  for(k=0; k<NUMBER_of_PLOTS; k++){
	...
	*/
	relative_coherence = value = 0.;
	if(point_Stochas == 7){
	  f_peak = Resonance_Frequency_Peak(&Parameters, 1);
	  overall_Amplification_Coherence(&Parameters, 1, f_peak, f_semiband, 
					  &value, &relative_coherence);
        }
	if(k == 0){
	  fprintf(fp,"%g\t%g\t%g\n", d_Rel, b_Rel, value);
    ...
    append_Fig_TEX(F_TEX, name, imm[k]);
    //ps_image("rel_coh.eps", Data, No_of_Points, Max_z, Min_z);
