...
void modelReport(char *File)
{
  FILE *fp;
	      ...
	      printf("      D = 0 (Without discarding Extinctions)\n");
	      printf("      D = 1 (Discarding Extinctions)\n");
	      ...
	      printf(" -W %d >> No. of Times to be dicard\n", TRANSIENT);
  ...
  fp = fopen(File, "w");
	   ...
	   fprintf(fp," -D %d  >> Disarding time series where there is an extinction:\n",
		     DISCARDING_EXTINCTIONS);
	   fprintf(fp, "      D = 0 (Without discarding Extinctions)\n");
	   fprintf(fp, "      D = 1 (Discarding Extinctions)\n");
	   ...
	   fprintf(fp, " -R %d >> No REPLICAS or Stochastic Realizations\n", 
		  Realizations);
	   fprintf(fp, " -K %d >> No. of segments in which the time series will be fragmented\n", 
		     k_Sets);
	   fprintf(fp, " -T %d >> No. of Times to be saved\n", TIMES);
	   fprintf(fp," -W %d >> No. of Times to be dicard\n", TRANSIENT);
